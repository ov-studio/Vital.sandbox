    ...
    Vital::Sandbox::Lua::API::bind(vm, "volumetric_fog", "setEmission", [](auto* ref) -> int {
        ...
        return vm -> execute([&]() -> int {
            if (vm -> isString(1)) {
                ...
                if (godot::Color::html_is_valid(value)) throw Vital::Error("invalid-arguments");
                Vital::Godot::Core::get_environment() -> set_volumetric_fog_emission(godot::Color::html(value));
            }
            else {
                if (vm -> getArgCount() < 4) throw Vital::Error("invalid-arguments");
                for (int i = 1; i <= 4; i++) {
                    if (!vm -> isNumber(i)) throw Vital::Error("invalid-arguments");
                }
