...
namespace Vital::System::Audio::Sound {
    ...
    create::create(std::string& path) {
        ...
        if (isErrored(vSystem -> createSound(path.data(), FMOD_DEFAULT, 0, &sound))) throw ErrorCodes["request_failed"];
    ...
    bool create::isLooping() {
        FMOD_MODE mode;
        if (isErrored(channel -> getMode(&mode))) throw ErrorCodes["request_failed"];
        return mode == FMOD_LOOP_NORMAL;
    }
    bool create::isVolumeRamped() {
        bool state = false;
        if (isErrored(channel -> getVolumeRamp(&state))) throw ErrorCodes["request_failed"];
        return state;
    }
    bool create::isMuted() {
        bool state = false;
        if (isErrored(channel -> getMute(&state))) throw ErrorCodes["request_failed"];
        return state;
    }

    // Setters //
    bool create::play() {
        if (isErrored(vSystem -> playSound(sound, nullptr, false, &channel))) throw ErrorCodes["request_failed"];
        return true;
    }
    bool create::play(FMOD::ChannelGroup* channelGroup) {
        if (isErrored(vSystem -> playSound(sound, channelGroup, false, &channel))) throw ErrorCodes["request_failed"];
        return true;
    }
    bool create::setChannelGroup(FMOD::ChannelGroup* channelGroup) {
        if (isErrored(channel -> setChannelGroup(channelGroup))) throw ErrorCodes["request_failed"];
        return true;
    }
    bool create::stop() {
        if (isErrored(channel -> stop())) throw ErrorCodes["request_failed"];
        return true;
    }
    bool create::setPaused(bool state) {
        if (isErrored(channel -> setPaused(state))) throw ErrorCodes["request_failed"];
        return true;
    }
    bool create::setLooped(bool state) {
        if (isErrored(channel -> setMode(FMOD_LOOP_NORMAL))) throw ErrorCodes["request_failed"];
        return true;
    }
    bool create::setPitch(float value) {
        if (isErrored(channel -> setPitch(value))) throw ErrorCodes["request_failed"];
        return true;
    }
    bool create::setVolume(float value) {
        if (isErrored(channel -> setVolume(value))) throw ErrorCodes["request_failed"];
        return true;
    }
    bool create::setVolumeRamped(bool state) {
        if (isErrored(channel -> setVolumeRamp(state))) throw ErrorCodes["request_failed"];
        return true;
    }
    bool create::setMuted(bool state) {
        if (isErrored(channel -> setMute(state))) throw ErrorCodes["request_failed"];
        return true;
    }
    bool create::set3DAttributes(Vital::Types::Math::Vector3D position, Vital::Types::Math::Vector3D velocity) {
        if (!is3D) throw ErrorCodes["invalid-3d-sound"];
        ...
        if (isErrored(channel -> set3DAttributes(&__position, &__velocity))) throw ErrorCodes["request_failed"];
        return true;
    }
    bool create::set3DConeSettings(Vital::Types::Audio::ConeSettings settings) {
        ...
        if (isErrored(channel -> set3DConeSettings(settings.insideAngle, settings.outsideAngle, settings.outsideVolume))) throw ErrorCodes["request_failed"];
        return true;
    }
    bool create::set3DConeOrientation(Vital::Types::Math::Vector3D orientation) {
        ...
        if (isErrored(channel -> set3DConeOrientation(&__orientation))) throw ErrorCodes["request_failed"];
        return true;
    }
    bool create::set3DDistanceFilter(Vital::Types::Audio::DistanceFilter filter) {
        if (!is3D) throw ErrorCodes["invalid-3d-sound"];
        if (isErrored(channel -> set3DDistanceFilter(filter.enable, filter.customLevel, filter.centerFrequency))) throw ErrorCodes["request_failed"];
        return true;
    }
    bool create::set3DDopplerLevel(float value) {
        if (isErrored(channel -> set3DDopplerLevel(value))) throw ErrorCodes["request_failed"];
        return true;
    }
    bool create::setPan(float value) {
        if (isErrored(channel -> setPan(value))) throw ErrorCodes["request_failed"];
        return true;
    }
    ...
    float create::getPitch() {
        float value = 0;
        if (isErrored(channel -> getPitch(&value))) throw ErrorCodes["request_failed"];
        return value;
    }
    float create::getAudibility() {
        float value = 0;
        if (isErrored(channel -> getAudibility(&value))) throw ErrorCodes["request_failed"];
        return value;
    }
    float create::getVolume() {
        float value = 0;
        if (isErrored(channel -> getVolume(&value))) throw ErrorCodes["request_failed"];
        return value;
    }
    bool create::get3DAttributes(Vital::Types::Math::Vector3D& position, Vital::Types::Math::Vector3D& velocity) {
        ...
        if (isErrored(channel -> get3DAttributes(&__position, &__velocity))) throw ErrorCodes["request_failed"];
    ...
    bool create::get3DConeSettings(Vital::Types::Audio::ConeSettings& settings) {
        if (!is3D) throw ErrorCodes["invalid-3d-sound"];
        if (isErrored(channel -> get3DConeSettings(&settings.insideAngle, &settings.outsideAngle, &settings.outsideVolume))) throw ErrorCodes["request_failed"];
        return true;
    }
    bool create::get3DConeOrientation(Vital::Types::Math::Vector3D& orientation) {
        if (!is3D) throw ErrorCodes["invalid-3d-sound"];
        FMOD_VECTOR __orientation;
        if (isErrored(channel -> get3DConeOrientation(&__orientation))) throw ErrorCodes["request_failed"];
    ...
    float create::get3DDopplerLevel() {
        if (!is3D) throw ErrorCodes["invalid-3d-sound"];
        float value = 0;
        if (isErrored(channel -> get3DDopplerLevel(&value))) throw ErrorCodes["request_failed"];
        return value;
    }
}