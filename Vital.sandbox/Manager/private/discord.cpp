...
namespace Vital::Manager {
    // Instantiators //
    Discord::Discord() {
        default_application_id = static_cast<uint64_t>(std::stoull(Manager::Module::fetch_json_value("config/discord", "application_id").as<std::string>()));
        default_activity = {
            Manager::Module::fetch_json_value("config/discord", "state").as<std::string>(),
            Manager::Module::fetch_json_value("config/discord", "details").as<std::string>()
        };
        reset_application();
    }
