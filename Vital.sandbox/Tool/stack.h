...
namespace Vital::Tool {
    ...
    struct StackValue {
        ...
        // Defined below — `Stack` is still incomplete here
        ...
        // Converters //
        // Defined below — all three reach into `Stack`, which is still incomplete here
        template<typename T = void>
    ...
    template<typename T>
    StackValue::StackValue(Stack v) : value(std::make_shared<Stack>(std::move(v))) {}

    template<typename T>
    godot::Variant StackValue::to_variant() const {
        return std::visit([](auto&& v) -> godot::Variant {
            ...
            else if constexpr (std::is_same_v<V, std::shared_ptr<Stack>>) return v ? godot::Variant(v -> to_dict()) : godot::Variant();
            return godot::Variant();
        }, value);
    }

    template<typename T>
    StackValue StackValue::from_variant(const godot::Variant& v) {
        switch (v.get_type()) {
    ...
    template<typename T>
    bool StackValue::operator==(const StackValue& other) const {
