...
namespace Vital::Tool::Shrinker {
    inline std::string compress(const std::string& input) {
        ...
        if (ZSTD_isError(res)) throw Tool::Log::fetch("request-failed", Tool::Log::Type::error, "unable to compress");
        output.resize(res);
        return output;
    }
    ...
    inline std::string decompress(const std::string& input) {
        ...
        if (frame_size == ZSTD_CONTENTSIZE_ERROR || frame_size == ZSTD_CONTENTSIZE_UNKNOWN) throw Tool::Log::fetch("request-failed", Tool::Log::Type::error, "unable to decompress");
        std::string output;
        output.resize(static_cast<size_t>(frame_size));
        size_t res = ZSTD_decompress(&output[0], output.size(), input.data(), input.size());
        if (ZSTD_isError(res)) throw Tool::Log::fetch("request-failed", Tool::Log::Type::error, "unable to decompress");
        output.resize(res);
        return output;
    }
}