...
namespace Vital::Sandbox::API {
    struct Database_Query : vm_module {
        inline static const std::vector<std::string> base_scope = {"core", "db_query"};
        ...
        static void methods(Machine* vm) {
            ...
            vm_module::bind_method<Instance>(vm, "fetch", [](auto vm, auto self, auto& id) -> int {
                ...
                Tool::Thread::create([instance_id, promise_id](Tool::Thread*) {
                    ...
                    try {
                        ...
                        Machine::enqueue([instance_id, promise_id, rows]() {
                            auto instance = Instance::find(instance_id);
            ...
            vm_module::bind_method<Instance>(vm, "alter", [](auto vm, auto self, auto& id) -> int {
                ...
                if (vm -> is_table(-1)) {
                    auto steps = API::Database::read_schema_actions(vm, vm -> get_count(), Tool::Database::SchemaActionType::Add);
                    actions.insert(actions.end(), steps.begin(), steps.end());
                }
                vm -> pop(1);
                vm -> table_get_value("modify", idx);
                if (vm -> is_table(-1)) {
                    auto steps = API::Database::read_schema_actions(vm, vm -> get_count(), Tool::Database::SchemaActionType::Modify);
                    actions.insert(actions.end(), steps.begin(), steps.end());
                }
                vm -> pop(1);
                ...
                if (vm -> is_table(-1)) {
                    ...
                    for (int i = 1; i <= vm -> get_length(step_index); i++) {
                        vm -> table_get_value(i, step_index);
                        if (vm -> is_string(-1)) {
                            Tool::Database::SchemaAction step;
                            step.type = Tool::Database::SchemaActionType::Drop;
