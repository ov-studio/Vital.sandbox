        ...
        static void methods(Machine* vm) {
            ...
            if constexpr (node_type == Type::Spatial || node_type == Type::Camera) {
                ...
                vm_module::bind_method<Instance>(vm, "get_quaternion", [](auto vm, auto self, auto& id) -> int {
                    vm -> push_value(self -> get_node() -> get_quaternion());
                    return 1;
                });
            }
            {
            ...
            if constexpr (node_type == Type::Spatial || node_type == Type::Camera) {
                ...
                API::Syncable::bind_method<Instance>(vm, "set_quaternion", [](auto vm, auto self, auto& id) -> int {
                    ...
                    self -> get_node() -> set_quaternion(vm -> get_quaternion(2));
                    vm -> push_value(true);
                    return 1;
                });
            }
            ...
            }
            {
                vm_module::bind_method<Instance>(vm, "set_parent", [](auto vm, auto self, auto& id) -> int {
                    ...
                    if (vm -> is_nil(2)) {
                        #if !defined(VSDK_Client)
                        if (self_is_server) self_syncable -> set_parent(nullptr);
                        #endif
                        {
