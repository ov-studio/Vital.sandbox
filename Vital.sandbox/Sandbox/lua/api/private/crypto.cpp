...
void Vital::Sandbox::Lua::API::Crypto::bind(void* instance) {
    ...
    API::bind(vm, "crypto", "encrypt", [](auto* ref) -> int {
        auto vm = fetchVM(ref);
        return vm -> execute([&]() -> int {
            if ((vm -> getArgCount() < 3) || (!vm -> isString(1)) || (!vm -> isString(2)) || (!vm -> isString(3))) throw Vital::Error("invalid-arguments");
            std::string mode = vm -> getString(1);
            std::string buffer = vm -> getString(2);
    ...
    API::bind(vm, "crypto", "decrypt", [](auto* ref) -> int {
        auto vm = fetchVM(ref);
        return vm -> execute([&]() -> int {
            if ((vm -> getArgCount() < 4) || (!vm -> isString(1)) || (!vm -> isString(2)) || (!vm -> isString(3)) || (!vm -> isString(4))) throw Vital::Error("invalid-arguments");
            std::string mode = vm -> getString(1);
            std::string buffer = vm -> getString(2);
            std::string key = vm -> getString(3);
