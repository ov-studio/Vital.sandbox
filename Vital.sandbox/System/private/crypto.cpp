...
namespace Vital::System::Crypto {
    std::pair<std::function<unsigned char* (const unsigned char*, size_t, unsigned char*)>, int> HashMode(const std::string& mode) {
        ...
        else throw std::runtime_error(get_error("hash-mode-nonexistent", mode));
    }
    ...
    const EVP_CIPHER* CipherMode(const std::string& mode) {
        ...
        else throw std::runtime_error(get_error("cipher-mode-nonexistent", mode));
    }
    ...
    std::string CipherHandle(const std::string& mode, bool isEncrypt, const std::string& buffer, const std::string& key, const std::string& iv) {
        try {
            ...
            if (EVP_CIPHER_CTX_key_length(ctx) != key.size()) throw std::runtime_error(get_error("cipher-invalid-key", key));
            if (EVP_CIPHER_CTX_iv_length(ctx) != iv.size()) throw std::runtime_error(get_error("cipher-invalid-iv", iv));
