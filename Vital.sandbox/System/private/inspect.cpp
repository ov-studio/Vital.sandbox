...
namespace Vital::System::Inspect {
    ...
    T toNumber(std::vector<T> value, int index = 0) {
        return ((index < 0) || value.empty() || (index > (value.size() - 1))) ? 0 : value.at(index);
    }

    std::wstring toString(std::vector<const wchar_t*> value, int index = 0, bool clipWhitespaces = false) {
        auto result = std::wstring(((index < 0) || value.empty() || (index > (value.size() - 1)) || (value.at(index) == nullptr)) ? L"" : value.at(index));
