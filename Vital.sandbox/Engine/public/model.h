...
namespace Vital::Engine {
    class Model : public godot::Node3D, public ISyncable {
        ...
        private:
            ...
            // Detached components — removed from the scene tree but kept alive
            // so they can be reattached later without reloading from the GLB.
            // Keyed by the same component path string used in find_mesh_node.
        ...
        public:
            ...
            const std::unordered_set<std::string>& get_components_hidden() const { return component_hidden; }
            const std::unordered_set<std::string>& get_components_detached() const { return component_detached; }
